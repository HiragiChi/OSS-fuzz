...
# then
# #   CONFIGURE_FLAGS="$CONFIGURE_FLAGS no-asm -fsanitize=cfi-icall -flto -fvisibility=hidden -fno-sanitize-trap=all -fsanitize-recover=all -ld=lld"
#     # CONFIGURE_FLAGS=CONFIGURE_FLAGS"-fsanitize=cfi -flto no-asm"
# fi
# echo "$CXX $CXXFLAGS"
# exit
if [[ $CFLAGS != *-m32* ]]
